...
public class LDAModel {
	...
	public LDAModel(String modelFilePath) throws IOException {
        ...
        int x = in.readInt(); //phi 矩阵行数
        int y = in.readInt(); //phi 矩阵列数
        phi = new double[x][y];
        for(int i = 0; i < x; i++) { //读取 phi 矩阵
        	for(int j = 0; j < y; j++) {
        		phi[i][j] = in.readDouble();
        	}
        }
        int wordListLength = in.readInt();
	...
	public static void saveModelFile(String saveModelFilePath, double[][] phi, Vocabulary vocabulary) throws IOException {
		...
		out.writeInt(phi.length); //phi 矩阵行数
		out.writeInt(phi[0].length); //phi 矩阵列数
		for (int i = 0; i < phi.length; i++) { //写入 phi 矩阵
		...
		out.writeInt(wordListLength); //词汇表长度
